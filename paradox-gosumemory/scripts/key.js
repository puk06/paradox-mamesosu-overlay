function drawKeyOverlay() {
    ...
    const VariablesEqual = () => {
        ...
        const objIsEqual = oldObjValue.every(
    ...
    const setupVariables = () => {
        ...
        document.documentElement.style.setProperty(
            "--keyglowwidth",
            `${key.thickness * 1.33 * 2}px`,
        );
        document.documentElement.style.setProperty(
            "--keyglowpositionX",
            `${key.size - key.thickness / 5 - key.thickness * 1.33}px`,
        );
        document.documentElement.style.setProperty(
            "--keymargin",
            `${key.margin}px`,
        ...
        document.documentElement.style.setProperty(
            "--keypositionX",
            `${key.size - key.thickness / 5}px`,
        );
        document.documentElement.style.setProperty(
            "--keypositionY2",
            `${key.thickness + key.margin}px`,
        );
        document.documentElement.style.setProperty(
            "--keypositionY3",
            `${(key.thickness + key.margin) * 2}px`,
        );
        document.documentElement.style.setProperty(
            "--keypositionY4",
            `${(key.thickness + key.margin) * 3}px`,
        );
        document.documentElement.style.setProperty(
            "--keyrotate",
            `rotate(${key.rotate}deg)`,
        );
        document.documentElement.style.setProperty(
            "--keynorotate",
            `rotate(${-key.rotate}deg)`,
        ...
        if (key.merge === true) {
            keyName = ["B1", "B2"];
            keyCanvas.height = keyCanvasCache.height =
                key.thickness * 2 + key.margin;
            document.documentElement.style.setProperty(
                "--keyheight",
                `${key.thickness * 2 + key.margin}px`,
            );
            document.documentElement.style.setProperty("--keymerge", `hidden`);
        } else {
            keyCanvas.height = keyCanvasCache.height =
                key.thickness * 4 + key.margin * 3;
            document.documentElement.style.setProperty(
                "--keyheight",
                `${key.thickness * 4 + key.margin * 3}px`,
            );
    ...
    const setKeyStatus = () => {
        ...
        if (key.merge === true) {
            keyStatus = [
                keyStatus[0] || keyStatus[2],
                keyStatus[1] || keyStatus[3],
            ];
        ...
        for (let i = 0; i < keyStatus.length; i++) {
            if (keyStatus[i] === true) {
                ctx_keyCache.fillRect(
                    keyCanvas.width - speed,
                    key.thickness * i + key.margin * i,
                    speed,
                    key.thickness,
                );
                if (key.all === true) {
                    document.getElementById(`k${i + 1}light`).style.opacity = 1;
                    document.getElementById(`k${i + 1}light`).style.filter =
                        "drop-shadow(0px 0px 10px var(--accentcolor)) drop-shadow(0px 0px 10px var(--accentcolor))";
                    document.getElementById(`k${i + 1}`).style.background =
                        "var(--accentcolor)";
                    document.getElementById(`k${i + 1}`).style.filter =
                        "brightness(160%) saturate(50%)";
                    document.documentElement.style.setProperty(
                        `--keypress${i + 1}`,
                        `0`,
                    );
                    ...
                    document.documentElement.style.setProperty(
                        ...
                        Math.min(
                            (key.thickness * 0.021) /
                                keyCount[i].toString().length,
                            (key.thickness * 0.023) / 3,
                        ) + "rem",
                    );
                }
            } else {
                ctx_keyCache.clearRect(
                    keyCanvas.width - speed,
                    key.thickness * i + key.margin * i,
                    speed,
                    key.thickness,
