const audioVisualizer = (() => {
    ...
    let speed;
    visualizer.style.filter =
        "drop-shadow(0px 0px 15px var(--accentcolorhalf)) drop-shadow(0px 0px 60px var(--accentcolor))";
    ...
    const observer = new MutationObserver(function (mutationsList, observer) {
        for (const mutation of mutationsList) {
            if (
                ...
                mutation.target.id === "currentbpm"
            ) {
                if (cache.currentBpm !== 0) {
                    speed = Math.max(
                        Math.min(
                            (1 - (cache.currentBpm * 2.5) / 1000) * 1.14,
                            0.97,
                        ),
                        0,
                    );
    ...
    function changeSaturation(rgb, amount) {
        ...
        return [newR, newG, newB].map((value) =>
            Math.round(Math.max(Math.min(value, 255), 0)),
        );
    }
    ...
    const VISUALIZER_VERTEX_SHADER_SOURCE = `#version 300 es
    ...
    const VISUALIZER_FRAGMENT_SHADER_SOURCE = `#version 300 es
    ...
    const visualizerProgram = createProgramFromSource(
        gl_v,
        VISUALIZER_VERTEX_SHADER_SOURCE,
        VISUALIZER_FRAGMENT_SHADER_SOURCE,
    );
    const visualizerUniforms = getUniformLocations(gl_v, visualizerProgram, [
        "u_resolution",
        "u_colors",
    ]);
    ...
    const renderVisualizer = () => {
        ...
        const setParticlesColor = () => {
            ...
            for (let i = 0; i < MaxNumX; i++) {
                const barValue = Math.floor(
                    frequencyData[
                        Math.floor(
                            (i * bufferLength) /
                                (MaxNumX *
                                    ((MaxNumX + freqencyCut * 100) / MaxNumX)),
                        )
                    ] **
                        2 /
                        (65025 / MaxNumY),
                ...
                for (let j = barValue - 1; j >= 0; j--) {
                    ...
                    const satulationColor = changeSaturation(
                        accentColor,
                        alphaJ,
                    );

                    color = satulationColor.map((value) =>
                        Math.round(
                            Math.max(
                                Math.min(value / (magnification / 0.6), 255),
                                0,
                            ),
                        ),
                    );
        ...
        const renderParticles = () => {
            ...
            gl_v.uniform4fv(
                colorLocation,
                new Float32Array(particlesColor.flat()),
            );
