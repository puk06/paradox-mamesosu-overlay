const shader = (() => {
    ...
    const COPY_FRAGMENT_SHADER_SOURCE = `#version 300 es
    ...
    const copyProgram = createProgramFromSource(
        ...
        COPY_FRAGMENT_SHADER_SOURCE,
    );
    const screenProgram = createProgramFromSource(
        gl,
        FILL_VIEWPORT_VERTEX_SHADER_SOURCE,
        SCREEN_FRAGMENT_SHADER_SOURCE,
    );
    const blurProgram = createProgramFromSource(
        gl,
        FILL_VIEWPORT_VERTEX_SHADER_SOURCE,
    ...
    const screenUniforms = getUniformLocations(gl, screenProgram, [
        "u_texture",
        "u_alpha",
    ]);
    const blurUniforms = getUniformLocations(gl, blurProgram, [
        "u_texture",
        "u_horizontal",
        "u_sampleStep",
    ]);

    const applyShaderToCanvas = function (
        destinationCanvas,
        reductionRate,
        radius,
        alpha = 1.0,
