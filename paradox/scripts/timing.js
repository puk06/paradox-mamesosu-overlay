function drawTimingBar() {
    ...
    const createGradient = function (color) {
        gradient = ctx_timing.createLinearGradient(
            0,
            timingCanvas.height / 2 - barThickness / 2,
            0,
            timingCanvas.height / 2 - hitErrorHeight,
        );
    ...
    const calculateTimingWindow = function () {
        ...
        if (Array.isArray(mods)) {
            if (
                cache.modsArray.includes("HR") ||
                cache.modsArray.includes("Hard Rock")
            ) {
                modOD = Math.min(OD * 1.4, 10);
            } else if (
                cache.modsArray.includes("EZ") ||
                cache.modsArray.includes("Easy")
            ) {
                modOD = OD / 2;
            }
        }
    ...
    const setupVariables = function () {
        timingBar.width =
            ...
            timingCanvas.height =
                1000 * timing.size;
        document.getElementById("timing").style.width = document.getElementById(
            "timing",
        ).style.height = area50 * 2 + 50 * timing.size + "px";
        document.documentElement.style.setProperty(
            "--timingcenter",
            -(timingCanvas.height / 2 - (area50 + 25 * timing.size)) + "px",
        );
        if (
            timing.rotate === 0 ||
            timing.rotate === 180 ||
    ...
    const createTimingBar = function () {
        ...
        ctx_timingBar.arc(
            timingCanvas.width / 2 - area50,
            timingCanvas.height / 2,
            barThickness,
            0,
            Math.PI * 2,
        );
        ...
        ctx_timingBar.fill();
        ctx_timingBar.fillRect(
            timingCanvas.width / 2 - area50,
            timingCanvas.height / 2 - barThickness,
            area50 * 2,
            barThickness * 2,
        );
        ctx_timingBar.fillStyle = "#58ff77";
        ctx_timingBar.fillRect(
            timingCanvas.width / 2 - area100,
            timingCanvas.height / 2 - barThickness,
            area100 * 2,
            barThickness * 2,
        );
        ...
        ctx_timingBar.fillRect(
            timingCanvas.width / 2 - hitErrorWidth / 2,
            timingCanvas.height / 2 - hitErrorHeight / 2,
            hitErrorWidth,
            hitErrorHeight,
        );
    };
    ...
    const renderHitErrors = function () {
        ...
        hitErrors.forEach((hitError) => {
            ...
            ctx_timing.fillRect(
                timingCanvas.width / 2 + hitError.offset - hitErrorWidth / 2,
                timingCanvas.height / 2 - hitErrorHeight / 2,
                hitErrorWidth,
                hitErrorHeight,
            );
    ...
    const setHitErrors = function () {
        ...
        const sum =
            currentHitErrors.slice(-5).reduce((acc, curr) => acc + curr, 0) / 5;
        if (timing.normalize === true) {
        ...
        if (currentHitErrors.length > previousHitErrors.length) {
            const addedItems = currentHitErrors.slice(
                Math.max(
                    previousHitErrors.length - currentHitErrors.length,
                    -5,
                ),
            );
