...
const audioControl = setInterval(() => {
    adjustedTime = adjustedTime = Math.round(((cache.time / 1000) + 0.1) * 100) / 100;
    timeDifference =
        Math.round(
...
function convertTosuDataForm(data) {
    return {
        audio: {
            fullPath: "",
        },
        ...
        mode: data.userProfile.playMode,
        ...
        ppIfMapEndsNow: data.gameplay.pp.current,
        mStars: data.menu.bm.stats.fullSR,
